...
Use the orderd sets to find the count of the values. 
...
void solve()
{
    ...
    */
    for(ll i=0;i<n;i++)
    {
        ...
        if(it!=s.end())
        {
            inside[v[i].second]=s.size()-s.order_of_key({v[i].first.second,0});
        }
    ...
    */
    ...
    {   
        ...
        if(it!=s.end())
        {
            contains[v[i].second]=s.size()-s.order_of_key({v[i].first.second,0});
        }
        s.insert({v[i].first.second,v[i].second});
    }
    //cout<<"\n";
